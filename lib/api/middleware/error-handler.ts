...
): (request: NextRequest, context?: unknown) => Promise<NextResponse<T | ErrorResponse>> {
  return async (request: NextRequest, context?: unknown): Promise<NextResponse<T | ErrorResponse>> => {
...
function formatZodErrors(error: unknown): Record<string, string[]> {
